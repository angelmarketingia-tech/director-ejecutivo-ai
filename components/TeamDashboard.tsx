...
import { Users, Play, CheckCircle2, ExternalLink, Loader2, Trash2, Clock, Activity } from "lucide-react";
...
export function TeamDashboard() {
  ...
  async function load() {
    try {
      ...
      if (j.ok) { setTeam(j.team); setStored(j.stored); }
    } catch {}
    setLoading(false);
  }
  ...
  return (
    <div className="panel p-4">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        {team.map((m) => (
          <MemberCard key={m.id} m={m} onAct={act} />
        ))}
      </div>
    </div>
  );
...
function MemberCard({ m, onAct }: { m: Member; onAct: (b: any) => Promise<void> }) {
