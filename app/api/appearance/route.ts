...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ok: true, looks: await getLooks(), role: u.role, me: u.id });
}
...
export async function POST(req: Request) {
  ...
  // Admin personaliza a cualquiera; cada miembro personaliza SU propio personaje.
  if (u.role !== "admin" && u.id.toLowerCase() !== id.toLowerCase()) {
    return NextResponse.json({ ok: false, error: "Solo puedes personalizar tu propio personaje." }, { status: 403 });
  }
