import Link from "next/link";

export default function NotFound() {
  return (
    <main className="relative flex min-h-screen items-center justify-center bg-bg px-4 text-center">
      <div className="aurora pointer-events-none fixed inset-0" />
      <div className="relative">
        <p className="glow-text text-[44px] font-bold tracking-tight text-text">NEXUS HQ</p>
        <p className="mt-2 text-[14px] text-text-muted">Esta página no existe o se movió.</p>
        <Link href="/" className="mt-5 inline-block rounded-lg bg-prospect/15 px-4 py-2.5 text-[13px] font-semibold text-prospect hover:bg-prospect/25">
          Volver al Centro de Mando
        </Link>
      </div>
    </main>
  );
}
